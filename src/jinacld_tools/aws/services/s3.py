...
from jinacld_tools.aws.helper import file_exists
...
class S3:
    ...
    def put(self, filepath, key):
        ...
        try:
            self.logger.info(
                f"Uploading object from `{filepath}` to S3 bucket `{self._bucket}` key `{key}`"
            )
            with open(filepath, "rb") as data:
                self._client.upload_fileobj(data, self._bucket, key)
        except Exception as exp:
            self.logger.error(
                f"Got following exception while uploading object to S3 \n{exp}"
            )
